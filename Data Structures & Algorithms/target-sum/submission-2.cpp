...
int solve(int i,int target,int posNegInd,vector<int>&nums,
vector<vector<vector<int>>>&dp){
	if(i < 0 and target !=0) return 0;
	if(target == 0 and i < 0) return 1;
	if(dp[i][abs(target)][posNegInd] != -1) return dp[i][abs(target)][posNegInd];
	
	return dp[i][abs(target)][posNegInd] = 
