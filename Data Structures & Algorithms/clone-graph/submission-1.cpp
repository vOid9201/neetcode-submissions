...
public:
    ...
    Node* cloneGraph(Node* node) {
        if(node == NULL) return NULL;
        unordered_map<int,Node*> pointsToNewNodes;
        Node* headCopyNode = new Node(node->val);
        pointsToNewNodes[node->val] = headCopyNode;
        queue<Node*> qt;
        qt.push(node);
        ...
        while(!qt.empty()){
            ...
            Node* copyNode = pointsToNewNodes[curr->val];
            
            for(auto x : curr->neighbors){
                if(pointsToNewNodes.find(x->val) == pointsToNewNodes.end()){
                    Node* newNode = new Node(x->val);
                    pointsToNewNodes[x->val] = newNode;
                    qt.push(x);
                }
                copyNode->neighbors.push_back(pointsToNewNodes[x->val]);
            }
        }
