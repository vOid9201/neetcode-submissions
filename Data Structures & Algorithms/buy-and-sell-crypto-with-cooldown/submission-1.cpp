...
public:
    ...
    int solve(int i,int sellBuyIndicator,vector<int>&prices, vector<vector<int>>&dp){
        //base condition 
        //sellBuyIndicator = 0 => sell  vice versa
        ...
        if(sellBuyIndicator == 0){
            ...
            dp[i][sellBuyIndicator] = max( (sold == INT_MIN ? INT_MIN : sold + prices[i]),
            notSold );
        }
        else{
            int bought = solve(i-2,0,prices,dp);
            int notBought = solve(i-1,1,prices,dp);
            dp[i][sellBuyIndicator] = max( (bought == INT_MIN ? INT_MIN : bought - prices[i]) ,
            notBought);
        }
