...
string curr,string par,
unordered_map<string,string> &parent,string actualParent){

	vis[curr][curr] = 1.00;
	parent[curr] = actualParent;
	
	for(auto x : adj[curr]){
		if(x.first != par){
			if(vis.find(x.first) == vis.end()) 
				dfs(adj,vis,x.first,curr,parent,actualParent);
...
vector<double> &values, vector<vector<string>> &queries){
	...
	for(auto x : adj){
		if(vis.find(x.first) == vis.end())
			dfs(adj,vis,x.first,"",parent,x.first);
	}
	...
	for(auto x : queries){
		...
		parent[x[0]] != parent[x[1]])
			...
			if(vis[x[0]].find(x[1]) != vis[x[0]].end())
				ans.push_back(vis[x[0]][x[1]]);
			else if(vis[x[1]].find(x[0]) != vis[x[1]].end())
				ans.push_back(1/vis[x[1]][x[0]]);
			else
